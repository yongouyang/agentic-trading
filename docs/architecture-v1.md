...
| **Bulk historical archive (one-off, paid)** | **Databento** XNAS.ITCH `ohlcv-1d` batch | US — all securities carried by the Nasdaq feed | API key (paid; batch download) | ✅ imported 2026-09-05 (`docs/research-databento-import.md`): 20,623 files → 11.33M bars 2021-09→2026-09 into **`VendorBar`** (as-traded, **not** R1-normalized — adjusted series derived on read via the **`SplitEvent`** registry: Yahoo v8 full-history events + FAR-tier in-band detections). ⚠️ feed is **not NASDAQ-only**: NYSE/Arca names arrive via ADF at *partial* volume and `publisher_id` cannot classify them — use `VendorInstrument` (nasdaqtraded.txt join). ⚠️ Reference/corporate-actions API 403s without a free portal subscribe — that's why split events came from Yahoo, not Databento |
