...
| **A5** | ✅ **DONE 2026-09-18** — CLI | `pnpm -C apps/api backtest:factor` → `reports/backtest/factor-<date>.{json,txt}` with per-alpha rows (mean 20d IC, ICIR, NW t + SE + lag, spread and proportional spread per horizon, by-year, breadth/day, warmup, declared `min_warmup_bars`, p, BH-adjusted p, `clearedFloor`, label) | met: 3 alphas run end-to-end on **both** lanes and the artifact renders. Two deviations from this row's flags, both recorded: `--zoo` is **not** a flag, because the signals are already computed and bound to the panel — the zoo digest travels in the bridge manifest instead, so a `--zoo` would be a knob that cannot change an answer; and `--universe u1\|u2\|both` was **added**, because fork 1 requires U2 reported alongside the primary U1 and the row's flags had no way to ask for it |
| **A6** | ✅ **DONE 2026-09-18** — The sweep | full US + HK sweep: `alpha-bridge.py` over all clean alphas (218 US + 166 HK, 0 skipped, 1.9 GB, 4 min), then `backtest:factor --market all` | met: every alpha carries a label (US 436 rows / HK 332 rows, none unlabelled, none skipped), both power floors and the luck benchmark printed per lane and per universe, artifact `reports/backtest/factor-2026-09-18.{json,txt}`. **Outcome: the US shortlist is EMPTY** — 0 of 436 US rows survive FDR, and no US alpha reaches its own luck benchmark (max ratio **0.78**) — so **A7 cannot run** |
| **A7** | 🔒 **BLOCKED 2026-09-18 — nothing to promote** | — (vendor panel + promoted-subset run not built) | The promotion rule is pre-registered as "the FDR-surviving, power-floor-clearing US alphas, ranked by mean 20d IC, capped at K = 20". A6 left that set **empty**, so there is no promoted subset to confirm and the vendor test half cannot be spent. This is the pre-registered rule meeting a null sweep, not a failed step; the honest action is to leave the half unspent rather than widen the rule after seeing outcomes. **The test half has now survived unspent twice** — Phase 4c declared the lane underpowered by design, and Phase 6A produced no shortlist |
